...
void PlaneInteractor::moveStart3D(mitk::BaseRenderer* sender, mitk::InteractionPositionEvent* positionEvent)
{
	...
	for (int i = 0; i < planeList->size(); i++)
	{
		...
		// 쓸데없는 코드........
		// PlaneGeometryData랑 연결된 Surface받아오려는 코드
...
void PlaneInteractor::move3D(mitk::BaseRenderer* sender, mitk::InteractionPositionEvent* positionEvent)
{
	...
	// Calculate 
	// PlaneObject를 Z방향으로 1Pixel 움직였을 때 World 좌표를, Display 좌표로 변환.
	// PlaneObject가 Z방향으로 1Pixel 움직일 때, Display 좌표계에서의 변화량을 얻기 위함.
	...
	// Mouse 이동량에서, 위에서 얻어온 방향vector의 0도 기준으로 X 변화량만 가져옴.
	// X, Y로 움직이려 할 때, 못움직이게 하기위함.
